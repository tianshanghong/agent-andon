...
description: "Agent Andon의 각 구성 요소를 시작, 확인, 중지하는 방법 — 보드 서버, 휴대폰 접속을 위한 Tailscale Serve, 그리고 선택적인 콘텐츠를 읽지 못하는 릴레이."
---
...
| **`andon relay`** | 8788 | 콘텐츠를 읽지 못하는 호스팅 릴레이 | **직접** 릴레이를 운영할 때만 — [deploy-relay.md](/ko/docs/deploy-relay/) 참고 |
...
> 실행하면 저희가 관리하는, 콘텐츠를 읽지 못하는 릴레이로 연결됩니다: 어디서나 보는 보드, 설정 제로, 호스팅할 것 없음.
> [Hosted Andon](/ko/docs/hosted/)을 참고하세요.

콘텐츠를 읽지 못하는 릴레이를 직접 호스팅할 때만 해당합니다(대부분은 관리형 릴레이나 Tailscale을 대신 사용합니다).
