...
<img src="docs/hero.png" width="880" alt="개발자의 책상: 왼쪽에는 터미널에서 Claude Code를 실행 중인 MacBook, 오른쪽 거치대에는 Agent Andon 보드를 띄운 스마트폰 — 위쪽에 빨간 「막힘」과 주황색 「확인 필요」 카드, 아래에 차분한 파란색 「작업 중」과 초록색 「완료」. AI 코딩 에이전트를 위한 상태 보드.">

*모든 AI 코딩 에이전트를 한 보드에서 — 당신이 필요한 것이 맨 위로 떠오릅니다. 왼쪽은 Claude Code와 Codex 세션, 오른쪽 폰에는 실시간 상태.*
...
## Claude Code나 Codex가 확인을 요청하거나, 끝나거나, 막히면 알림 받기

Agent Andon은 **Claude Code**와 **OpenAI Codex**의 네이티브 라이프사이클 훅을 설치해 각 턴을 한눈에 보이는 신호로 바꿉니다 — 보드(어떤 화면이든), 데스크톱 배너, 휴대폰 푸시로. 더 이상 터미널을 지켜보며 입력을 기다리지 않아도 됩니다.

## 여러 Claude Code / Codex 에이전트를 한 번에 지켜보기

여러 세션을 동시에 돌리고 에이전트마다 한 줄로 봅니다. **당신을 필요로 하는** 것이 맨 위로 떠오릅니다. 여러 개를 돌려도 정말 필요할 때만 보면 됩니다.

## 기본적으로 셀프 호스팅이고 프라이빗

순수 Node 표준 라이브러리, 런타임 의존성 0, 계정 없음, 텔레메트리 없음 — 당신의 머신에서 돌아갑니다. 선택적 콘텐츠 블라인드 릴레이(복호화할 수 없는 암호문만 저장)를 쓰면 코드를 노출하지 않고도 어디서나 보드와 휴대폰 푸시를 쓸 수 있습니다.
...
`andon serve`로 보드 전체를 로컬에서 실행하거나(무료, 데스크톱 알림, 릴레이 불필요), `andon relay`로 직접 릴레이를 운영하세요(`andon verify <url>`로 어떤 릴레이든 검증). → [전체 셀프호스트 가이드](docs/deploy-relay.md)
