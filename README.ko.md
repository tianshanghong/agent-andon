...
andon relay                                        # …또는 제로 지식 릴레이를 직접 실행
andon verify <relay-url>                           # 릴레이가 바로 그 오픈소스 코드를 제공하는지 확인
```

모든 상태는 기기를 벗어나기 전에 **당신의 기기에서 종단 간 암호화**됩니다. 릴레이는 **암호문만** 라우팅하고
저장하며, 당신의 프롬프트나 코드, 제목, 메시지는 읽지 못합니다 — 당신이 활동 중이라는 사실과 대략적인 시각,
그리고 IP만 볼 수 있습니다. *"단순히 신뢰하는 게 아니라, 검증할 수 있다":* 제공되는 코드는 오픈소스이며 재현
