# Hosted Andon — "board from anywhere" (user guide)
...
> Deploying a relay for others to share? See **[deploy-relay.md](deploy-relay.md)**.
...
into chat; save it in a password manager. (A scan-to-pair QR is planned to remove copy-pasting entirely.)
...
- **On your phone / another device:** the relay must be reachable over **HTTPS** (browsers require a secure
  ...
  - **A real domain + cert** (for a shared relay) — see [deploy-relay.md](deploy-relay.md).
