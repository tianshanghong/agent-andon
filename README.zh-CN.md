...
<img src="docs/hero.png" width="880" alt="开发者的桌面：左边是一台在终端里运行 Claude Code 的 MacBook，右边支架上的手机显示着 Agent Andon 看板——顶部是红色「卡住」和琥珀色「需要你」卡片，下方是更平静的蓝色「运行中」和绿色「完成」。一块给 AI 编码 agent 的状态看板。">

*所有 AI 编码 agent，一块板子看全——需要你的那个会浮到最上面。左边是 Claude Code 与 Codex 会话，右边手机上是它们的实时状态。*
...
## 当 Claude Code 或 Codex 需要你、完成或卡住时，第一时间通知你

Agent Andon 装好 **Claude Code** 和 **OpenAI Codex** 的原生生命周期钩子，把每一轮变成一眼可见的信号——看板（任意屏幕）、桌面横幅或手机推送——你不必再盯着终端等它要你确认。

## 同时盯着多个 Claude Code / Codex agent

并行跑多个会话，每个 agent 一行；**需要你**的那个自动浮到最上面。养一整队 agent，只在真有一个需要你时才看一眼。

## 默认自托管、默认私有

纯 Node 标准库、零运行时依赖、无账号、无遥测——跑在你自己机器上。可选的内容盲中继（只存它无法解密的密文）让你随处看板、随处收手机推送，而不暴露你的代码。
...
用 `andon serve` 在本地跑完整看板（免费、带桌面提醒、不需要中继）；或用 `andon relay` 跑你自己的中继（`andon verify <url>` 可校验任意中继）。→ [完整自托管指南](docs/deploy-relay.md)
